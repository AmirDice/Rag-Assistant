...
def test_glossary_seeds_from_yaml(isolated_glossary):
    ...
    assert "encargo" in terms
    assert "HCP" in terms


def test_preprocess_enriches_with_canonical_term(isolated_glossary):
    pre = asyncio.run(preprocess_query("como hago un enargo de cliente", tenant_id="demo"))
    # "enargo" is a known typo alias of canonical "encargo".
    assert "encargo" in pre.matched_terms
    assert "encargo" in pre.retrieval_query
    assert pre.ai_used is False
    assert pre.original == "como hago un enargo de cliente"


def test_preprocess_acronym_expansion(isolated_glossary):
    pre = asyncio.run(preprocess_query("error con hcp", tenant_id="demo"))
    assert "HCP" in pre.matched_terms
    # expansion tokens widen lexical recall
    assert "hoja" in normalize_text(pre.retrieval_query)
...
def test_preprocess_disabled_passthrough(isolated_glossary, monkeypatch):
    ...
    pre = asyncio.run(preprocess_query("como hago un enargo", tenant_id="demo"))
    assert pre.retrieval_query == "como hago un enargo"
    assert pre.matched_terms == []
